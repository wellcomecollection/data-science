...
def get_image_url_from_iiif_url(iiif_url, size=(224, 224)):
    try:
    ...
    if "dlcs" in image.api_endpoint:
        # DLCS provides a thumbnails service which only serves certain sizes
        # of image. Requests for these don't touch the image server and so, as
    ...
    return str(image.size(width=size[0], height=size[1], exact=False))
